...
/* USER CODE BEGIN PFP */
void user_init(void);			// è¿½åŠ?ã‚³ãƒ¼ãƒ?
void user_main_loop(void);		// è¿½åŠ?ã‚³ãƒ¼ãƒ?
/* USER CODE END PFP */
...
static void MX_TIM1_Init(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim1);    // è¿½åŠ?ã‚³ãƒ¼ãƒ‰ï¼šã‚¿ã‚¤ãƒžã?¼?¼‘ã?®å‹•ä½œé–‹å§?
